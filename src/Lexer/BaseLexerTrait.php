...
trait BaseLexerTrait {
  ...
  /**
   * Consumes all leading 0x20 bytes from the input stream.
   *
   * A lexical error may occur as a consequence of calling this function upon
   * reading zero bytes from the input stream if $empty_error was supplied.
   *
   * @param \Psr\Http\Message\StreamInterface $input
   *   The input stream from which to consume 0x20 bytes.
   * @param string|null $empty_error
   *   The error message to use if the result is empty (default: NULL).
   */
  protected function consumeSpace(StreamInterface $input, ?string $empty_error = NULL): void {
    $this->read($input, ' ', $empty_error);
  }
